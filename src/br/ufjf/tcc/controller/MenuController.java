...
public class MenuController extends CommonsController {
			...
			if (getCurrentCalendar() != null) {
				...
				if(getUsuario().isAtivo()) {
					if(tccUsuario != null && tccUsuario.getStatus() == TCC.APROVADO) {
						Messagebox.show("Você já enviou a versão final de seu trabalho, portanto não pode modificá-lo.",
	...
	public void sendMail(@BindingParam("email") String email,
		...
		// Verfica se o usuÃ¡rio realmente existe
	...
	public String getMeuX()// diz para o usuario aluno se eles estÃ¡ mechendo em
	...
	@Command
	public void projetosTrabalhosSemestre() // pagina com as informaÃ§Ãµes para
											// o coordenador
	{
	...
	@Command
	public void trabalhos()// informaÃ§oes dos projetos do curso
	{
		...
		if(senhaAntiga!=null){
			if(senhaNova1!=null){
				if(senhaNova1.length()>5){
					...
					errorLbl.setValue("Senha deve ter no mínimo 6 caracteres");
					errorLbl.setVisible(true);
				}
			}else{
