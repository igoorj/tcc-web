	...
	public void submit() {
		...
		if (userTemp != null) {
			if (userTemp.getCurso().getIdCurso() != getUsuario().getCurso()
					.getIdCurso()) {
				Messagebox
						.show("Matrícula de usuário já existe e pertence a um curso diferente.",
								"Erro", Messagebox.OK, Messagebox.ERROR);
				return;
			} else if (userTemp.getTipoUsuario().getIdTipoUsuario() != Usuario.ALUNO) {
				Messagebox
						.show("Matrícula de usuário já existe e não é de aluno.",
								"Erro", Messagebox.OK, Messagebox.ERROR);
				return;
			} else {
				for (TCC tcc : tccsSemestre) {
					if (tcc.getAluno().getMatricula()
						...
						Messagebox
								.show("Usuário já possui um Trabalho no Semestre Atual",
										"Erro", Messagebox.OK, Messagebox.ERROR);
						return;
					}
				}
