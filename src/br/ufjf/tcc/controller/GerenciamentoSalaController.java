				...
				public void onEvent(Event event) throws Exception {
					if (salaBusiness.validate(auxSala)) {
						if (salaBusiness.salvaOuEdita(auxSala)) {
							...
							Clients.clearBusy(window);
							Messagebox.show("Sala nÃ£o foi adicionada! Ocorreu um erro no banco", "Erro", Messagebox.OK,
									Messagebox.ERROR);
						}
						...
						for (String error : salaBusiness.getErrors())
							errorMessage += error;
						Clients.clearBusy(window);
						Messagebox.show(errorMessage, "Dados insuficientes / invÃ¡lidos", Messagebox.OK,
								Messagebox.ERROR);
					}
				}
			});
