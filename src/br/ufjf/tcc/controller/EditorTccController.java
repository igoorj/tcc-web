...
public class EditorTccController extends CommonsController {
	...
	public void init() {
		...
		default:
			...
			if (tcc == null || !canEdit()) 
			 ...
			 else if(!(tipoUsuario == Usuario.SECRETARIA && idCursoOrigem == 5)) {  // Tratamento for�ado
					verificarAtrasado();
			 }
		}
	...
	public void setTempLinkSala(String tempLinkSala) {
		...
		int tipoUsuario = getUsuario().getTipoUsuario().getIdTipoUsuario();
		if(tcc.getStatus() >= TCC.TEPB && tipoUsuario != Usuario.COORDENADOR) {
			Messagebox.show("N�o � poss�vel alterar o link da sala depois de ter marcado a defesa", "Opera��o inv�lida", Messagebox.OK,
					Messagebox.ERROR);
	...
	public void setTempDataApresentacao(Timestamp tempDataApresentacao) {
		...
		if(!tccBusiness.validateDataApresentacao(tempDataApresentacao, tcc.getCalendarioSemestre())) {
			Messagebox.show("A data informada est� fora do prazo de apresenta��o", "Data inv�lida", Messagebox.OK,
					Messagebox.ERROR);
			tcc.setDataApresentacao(null);
		}
		if(tcc.getStatus() >= TCC.TEPB) {
			Messagebox.show("N�o � poss�vel alterar a data de apresenta��o depois de ter marcado a defesa", "Opera��o inv�lida", Messagebox.OK,
					Messagebox.ERROR);
			tempDataApresentacao = tcc.getDataApresentacao();
			return;
		}
